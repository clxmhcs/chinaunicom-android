 ...
 * Password login stays hidden because the frozen iOS source currently disables it.
 */
@Composable
    ...
    LazyColumn(
        ...
        item {
            FunctionalCard(title = "联通号码") {
                if (accounts.isEmpty()) {
                    Text("当前没有正式账号。新增成功后会立即同步到首页、语音和综合业务。")
                } else {
                    Text("已保存 ${accounts.size} 个号码")
                }
            }
        }
        ...
        if (accounts.isNotEmpty()) {
            items(accounts, key = { it.id }) { account ->
                FunctionalCard(title = account.displayName) {
                    Text(account.mobile)
                    Text(account.packageName ?: "套餐信息暂无")
                }
            }
        }
        ...
        item {
            FunctionalCard(title = "从 iOS 导入凭据") {
                Text("可直接选择此前 M4 联网验收使用的 iOS version-1 凭据 JSON。文件只在本机读取；Cookie、appId、token_online 会经过真实余量验证后写入 Android Keystore，不写入普通账号文件。")
                Button(
        ...
        item {
            FunctionalCard(title = "短信验证码登录") {
                Text("与 iOS 当前可用登录入口一致。验证码只用于本次请求，不会保存。密码登录协议底层已迁移，但 iOS 当前仍禁用该 UI，因此 Android 也不开放。")
                OutlinedTextField(
                    modifier = Modifier.fillMaxWidth(),
                    value = mobile,
        ...
        onboarding.captchaChallenge?.let { challenge ->
            item {
                FunctionalCard(title = challenge.title) {
                    ...
                    Text("该号码触发了联通风险安全验证。底层 challenge/resultToken 续接协议已经存在；当前先使用上方 iOS 凭据导入路径完成正式账号迁移，风险验证交互页留到账号页面 UI 阶段接线。")
                }
            }
        }
        ...
        if (onboarding.statusTitle != null || onboarding.statusMessage != null) {
            item {
                FunctionalCard(title = onboarding.statusTitle ?: "状态") {
                    ...
                    Button(onClick = flowViewModel::clearOnboardingStatus) {
                        Text("清除状态")
                    }
                }
            }
        ...
        item {
            FunctionalCard(title = "其它设置") {
                Text("刷新策略、Widget、自动化和最终设置页视觉布局继续按迁移计划后续接入；本阶段只补账号真实数据链路。")
            }
        }
    }
}
