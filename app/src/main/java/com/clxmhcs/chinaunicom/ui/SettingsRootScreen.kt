...
class SettingsRootViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun cycleDisplayUnit() = saveAppSettings {
        copy(displayUnit = when (displayUnit) {
    ...
    fun addBalanceGroup() { viewModelScope.launch { unicomRepository.addBalanceAccountGroup() } }
    fun deleteBalanceGroup(groupID: UUID) { viewModelScope.launch { unicomRepository.deleteBalanceAccountGroup(groupID) } }
    ...
    fun setDefaultFinancialAccount(accountID: UUID?, groupID: UUID) = unicomRepository.setDefaultFinancialAccountID(accountID, groupID)

    fun setQuotaAutomaticRefresh(value: Boolean) = refreshRepository.saveQuotaRefreshPolicy(quotaRefreshPolicy.value.copy(automaticRefreshEnabled = value))
    fun setQuotaColdLaunch(value: Boolean) = refreshRepository.saveQuotaRefreshPolicy(quotaRefreshPolicy.value.copy(refreshOnColdLaunch = value))
    fun setQuotaForeground(value: Boolean) = refreshRepository.saveQuotaRefreshPolicy(quotaRefreshPolicy.value.copy(refreshOnForeground = value))
    fun changeQuotaInterval(delta: Int) = refreshRepository.saveQuotaRefreshPolicy(quotaRefreshPolicy.value.copy(minimumIntervalMinutes = (quotaRefreshPolicy.value.minimumIntervalMinutes + delta).coerceIn(1, 1440)))
    fun changeQuotaAccountGap(delta: Int) = refreshRepository.saveQuotaRefreshPolicy(quotaRefreshPolicy.value.copy(accountGapSeconds = (quotaRefreshPolicy.value.accountGapSeconds + delta).coerceIn(0, 60)))

    fun setBalanceAutomaticRefresh(value: Boolean) = refreshRepository.saveBalanceRefreshPolicy(balanceRefreshPolicy.value.copy(automaticRefreshEnabled = value))
    fun setBalanceForeground(value: Boolean) = refreshRepository.saveBalanceRefreshPolicy(balanceRefreshPolicy.value.copy(checkOnForeground = value))
    fun changeBalanceInterval(delta: Int) = refreshRepository.saveBalanceRefreshPolicy(balanceRefreshPolicy.value.copy(intervalMinutes = (balanceRefreshPolicy.value.intervalMinutes + delta).coerceIn(1, 1440)))
    fun changeBalanceFailureRetry(delta: Int) = refreshRepository.saveBalanceRefreshPolicy(balanceRefreshPolicy.value.copy(failureRetryMinutes = (balanceRefreshPolicy.value.failureRetryMinutes + delta).coerceIn(1, 1440)))

    fun cycleOrderedBusinessEntryMode() = refreshRepository.saveOrderedBusinessRefreshPolicy(orderedBusinessRefreshPolicy.value.copy(entryMode = nextCachedMode(orderedBusinessRefreshPolicy.value.entryMode)))
    fun changeOrderedBusinessCache(delta: Int) = refreshRepository.saveOrderedBusinessRefreshPolicy(orderedBusinessRefreshPolicy.value.copy(cacheValidityHours = (orderedBusinessRefreshPolicy.value.cacheValidityHours + delta).coerceIn(1, 720)))
    fun setOrderedBusinessNoCacheQuery(value: Boolean) = refreshRepository.saveOrderedBusinessRefreshPolicy(orderedBusinessRefreshPolicy.value.copy(noCacheAutoQuery = value))
    fun changeOrderedBusinessGap(delta: Int) = refreshRepository.saveOrderedBusinessRefreshPolicy(orderedBusinessRefreshPolicy.value.copy(refreshAllAccountGapSeconds = (orderedBusinessRefreshPolicy.value.refreshAllAccountGapSeconds + delta).coerceIn(0, 60)))

    fun cycleMyPackageEntryMode() = refreshRepository.saveMyPackageRefreshPolicy(myPackageRefreshPolicy.value.copy(entryMode = nextPageEntryMode(myPackageRefreshPolicy.value.entryMode)))
    fun changeMyPackageCache(delta: Int) = refreshRepository.saveMyPackageRefreshPolicy(myPackageRefreshPolicy.value.copy(cacheValidityMinutes = (myPackageRefreshPolicy.value.cacheValidityMinutes + delta).coerceIn(1, 1440)))

    fun changePhoneBillCurrentCache(delta: Int) = refreshRepository.savePhoneBillRefreshPolicy(phoneBillRefreshPolicy.value.copy(currentMonthCacheMinutes = (phoneBillRefreshPolicy.value.currentMonthCacheMinutes + delta).coerceIn(1, 1440)))
    fun changePhoneBillHistoricalCache(delta: Int) = refreshRepository.savePhoneBillRefreshPolicy(phoneBillRefreshPolicy.value.copy(historicalCacheDays = (phoneBillRefreshPolicy.value.historicalCacheDays + delta).coerceIn(1, 365)))
    fun changePhoneBillRecheckDay(delta: Int) = refreshRepository.savePhoneBillRefreshPolicy(phoneBillRefreshPolicy.value.copy(monthlyRecheckDay = (phoneBillRefreshPolicy.value.monthlyRecheckDay + delta).coerceIn(1, 28)))
    fun changePhoneBillRecheckHour(delta: Int) = refreshRepository.savePhoneBillRefreshPolicy(phoneBillRefreshPolicy.value.copy(monthlyRecheckHour = (phoneBillRefreshPolicy.value.monthlyRecheckHour + delta).coerceIn(0, 23)))

    fun setIntegralAutomatic(value: Boolean) = refreshRepository.saveIntegralRefreshPolicy(integralRefreshPolicy.value.copy(automaticRefreshEnabled = value))
    fun cycleIntegralMode() = refreshRepository.saveIntegralRefreshPolicy(integralRefreshPolicy.value.copy(cycleMode = nextIntegralMode(integralRefreshPolicy.value.cycleMode)))
    fun changeIntegralMonthlyDay(delta: Int) = refreshRepository.saveIntegralRefreshPolicy(integralRefreshPolicy.value.copy(monthlyRefreshDay = (integralRefreshPolicy.value.monthlyRefreshDay + delta).coerceIn(1, 28)))
    fun changeIntegralMonthlyHour(delta: Int) = refreshRepository.saveIntegralRefreshPolicy(integralRefreshPolicy.value.copy(monthlyRefreshHour = (integralRefreshPolicy.value.monthlyRefreshHour + delta).coerceIn(0, 23)))
    fun changeIntegralFixedHours(delta: Int) = refreshRepository.saveIntegralRefreshPolicy(integralRefreshPolicy.value.copy(fixedIntervalHours = (integralRefreshPolicy.value.fixedIntervalHours + delta).coerceIn(1, 720)))
    fun setIntegralCheckOnEntry(value: Boolean) = refreshRepository.saveIntegralRefreshPolicy(integralRefreshPolicy.value.copy(checkOnEntry = value))

    fun setOrderRefreshOnEntry(value: Boolean) = refreshRepository.saveOrderRefreshPolicy(orderRefreshPolicy.value.copy(refreshOnEntry = value))
    fun setRebateGiftAutomatic(value: Boolean) = refreshRepository.saveRebateGiftRefreshPolicy(rebateGiftRefreshPolicy.value.copy(automaticRefreshEnabled = value))
    fun changeRebateGiftDay(delta: Int) = refreshRepository.saveRebateGiftRefreshPolicy(rebateGiftRefreshPolicy.value.copy(monthlyRefreshDay = (rebateGiftRefreshPolicy.value.monthlyRefreshDay + delta).coerceIn(1, 28)))
    fun changeRebateGiftHour(delta: Int) = refreshRepository.saveRebateGiftRefreshPolicy(rebateGiftRefreshPolicy.value.copy(monthlyRefreshHour = (rebateGiftRefreshPolicy.value.monthlyRefreshHour + delta).coerceIn(0, 23)))
    fun setRebateGiftNoCacheQuery(value: Boolean) = refreshRepository.saveRebateGiftRefreshPolicy(rebateGiftRefreshPolicy.value.copy(queryImmediatelyWhenNoCache = value))
...
@Composable
private fun BalanceGroupingEditor(accounts: List<UnicomAccount>, groups: List<BalanceAccountGroup>, settings: AppSettings, onAddGroup: () -> Unit, onDeleteGroup: (UUID) -> Unit, onToggleMember: (UUID, UUID) -> Unit) {
    SettingsSection("合账号码选择") {
        ...
        groups.forEach { group ->
            ...
            accounts.forEach { account -> SettingsSwitchRow(displayMobileNumber(account.mobile, settings), account.id in group.memberAccountIDs) { onToggleMember(account.id, group.id) } }
        }
    }
}

@Composable
private fun FinancialRefreshEditor(accounts: List<UnicomAccount>, groups: List<BalanceAccountGroup>, homeAccountID: UUID?, settings: AppSettings, onSetHome: (UUID?) -> Unit, onSetGroupDefault: (UUID?, UUID) -> Unit) {
    SettingsSection("余额 / 账单 刷新号码") {
        ...
        accounts.forEach { account -> SettingsActionRow(displayMobileNumber(account.mobile, settings), if (account.id == homeAccountID) "当前首页余额号码" else "设为首页余额号码") { onSetHome(account.id) } }
        if (homeAccountID != null) TextButton(onClick = { onSetHome(null) }) { Text("取消首页余额号码") }
        groups.filter { it.memberAccountIDs.size >= 2 }.forEach { group ->
            ...
            group.memberAccountIDs.mapNotNull { id -> accounts.firstOrNull { it.id == id } }.forEach { account ->
                SettingsActionRow(displayMobileNumber(account.mobile, settings), if (group.defaultAccountID == account.id) "已保存为组默认" else "设为组默认") { onSetGroupDefault(account.id, group.id) }
            }
        }
    }
}
...
fun AppRefreshLogicSettingsScreen(settingsViewModel: SettingsRootViewModel, onBack: () -> Unit) {
    ...
    LazyColumn(modifier = Modifier.fillMaxSize(), contentPadding = PaddingValues(16.dp), verticalArrangement = Arrangement.spacedBy(14.dp)) {
        item { Row(verticalAlignment = Alignment.CenterVertically) { TextButton(onClick = onBack) { Text("返回") }; Text("App刷新逻辑", style = MaterialTheme.typography.headlineSmall, fontWeight = FontWeight.Bold) } }
        item { SettingsSection("首页与基础数据 · 流量 / 语音") {
        ...
        item { SettingsSection("我的订单") { SettingsSwitchRow("进入页面自动刷新", orders.refreshOnEntry, settingsViewModel::setOrderRefreshOnEntry) } }
        item { SettingsSection("返费 / 赠费") {
...
@Composable
private fun AccountOrderRow(title: String, canMoveUp: Boolean, canMoveDown: Boolean, onMoveUp: () -> Unit, onMoveDown: () -> Unit) {
    Row(modifier = Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) { Text(title, modifier = Modifier.weight(1f)); TextButton(onClick = onMoveUp, enabled = canMoveUp) { Text("上移") }; TextButton(onClick = onMoveDown, enabled = canMoveDown) { Text("下移") } }
}

@Composable
private fun SettingsSection(title: String, content: @Composable ColumnScope.() -> Unit) {
    Surface(modifier = Modifier.fillMaxWidth(), shape = MaterialTheme.shapes.large, tonalElevation = 1.dp) { Column(modifier = Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) { Text(title, style = MaterialTheme.typography.titleMedium, fontWeight = FontWeight.SemiBold); content() } }
}

@Composable
private fun SettingsSwitchRow(label: String, checked: Boolean, onCheckedChange: (Boolean) -> Unit) {
    Row(modifier = Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) { Text(label, modifier = Modifier.weight(1f)); Switch(checked = checked, onCheckedChange = onCheckedChange) }
}

@Composable
private fun SettingsActionRow(label: String, detail: String, onClick: () -> Unit) {
    OutlinedButton(modifier = Modifier.fillMaxWidth(), onClick = onClick) { Column(modifier = Modifier.fillMaxWidth()) { Text(label); Text(detail, style = MaterialTheme.typography.labelSmall) } }
}

@Composable
private fun SettingsDeferredRow(label: String, stage: String) {
    Surface(modifier = Modifier.fillMaxWidth(), color = MaterialTheme.colorScheme.surfaceVariant.copy(alpha = 0.35f)) { Row(modifier = Modifier.padding(10.dp), verticalAlignment = Alignment.CenterVertically) { Text(label, modifier = Modifier.weight(1f)); Text(stage, style = MaterialTheme.typography.labelSmall, color = MaterialTheme.colorScheme.onSurfaceVariant) } }
}

@Composable
private fun NumberEditor(label: String, value: Int, unit: String, step: Int, onChange: (Int) -> Unit) {
    Row(modifier = Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) { Column(modifier = Modifier.weight(1f)) { Text(label); Text("$value $unit", style = MaterialTheme.typography.labelSmall) }; TextButton(onClick = { onChange(-step) }) { Text("-$step") }; TextButton(onClick = { onChange(step) }) { Text("+$step") } }
}

private fun cachedModeTitle(mode: CachedBusinessEntryMode) = when (mode) {
...
private fun pageModeTitle(mode: PageEntryRefreshMode) = when (mode) {
...
private fun integralModeTitle(mode: IntegralRefreshCycleMode) = when (mode) {
