...
class UnicomAPIClient(
    ...
    private data class ActivatedSession(val cookie: String, val tokenOnline: String)
    ...
    companion object {
        ...
        const val ONLINE_URL = "$BASE_URL/mobileService/onLine.htm"
    ...
    fun activateSession(credentials: AccountCredentials): AccountCredentials {
        ...
        val activated = activateManualSession(cookie, appID, tokenOnline)
        return AccountCredentials(activated.cookie, appID, activated.tokenOnline)
    }
    ...
    fun fetchBalance(credentials: AccountCredentials): BalanceFetchResult {
        ...
        return try {
            ...
            val activated = activateManualSession(cookie, appID, tokenOnline)
            val changed = activated.cookie != cookie || activated.tokenOnline != tokenOnline
            ...
            BalanceFetchResult(
                ...
                updatedCredentials = if (changed) AccountCredentials(activated.cookie, appID, activated.tokenOnline) else null,
            )
        }
    }
    ...
    private fun fetchQuota(
        ...
        return try {
            ...
            val tokenOnline = credentials.tokenOnline.trimmedOrNull() ?: throw directError
            val activated = try {
                activateManualSession(cookie, appID, tokenOnline)
            ...
            val changed = activated.cookie != cookie || activated.tokenOnline != tokenOnline
            if (changed) result.copy(updatedCredentials = AccountCredentials(activated.cookie, appID, activated.tokenOnline)) else result
        }
    }
    ...
    private fun activateManualSession(
        ...
        return ActivatedSession(activatedCookie, activatedToken)
    }
